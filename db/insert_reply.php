...
for ($i=0; $i < 200; $i++){
  $id = $commentsIds[mt_rand(0, count($commentsIds) - 1)];
  $name = $nicknames[ mt_rand(0, count($nicknames)-1 )];
  $reply = $replys[ mt_rand(0, count($replys)-1 )];

  $postReply = "INSERT INTO `sub_comments` (`id`, `comment_id`, `nickname`, `sub_content`, `created_at`) VALUES (NULL, '$id', '$name', '$reply', CURRENT_TIMESTAMP)";
